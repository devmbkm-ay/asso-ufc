...
class PushSubscription(Base):
    # Abonnement Web Push d'un navigateur/appareil pour un membre — un membre
    # peut en avoir plusieurs (téléphone + ordinateur). `endpoint` identifie
    # le service de push du navigateur (FCM, Mozilla...), unique par
    # installation ; `p256dh`/`auth` sont les clés de chiffrement fournies
    # par `PushSubscription.toJSON()` côté navigateur. Consommé par
    # notify_member() dans app/core/notifications.py.
